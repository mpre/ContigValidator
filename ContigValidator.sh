...
-t | --threads int -> number of threads used\n\
...
cv_st_based $multipleGenomeFile $tmpsuffixfile $inputFiles > $tmpoutfile 2> $tmperrorfile
...
if [ "$bwaskip" = 0 ]; then
    ...
    do
	output="$i.bwa.bam"
	bwa mem -t $threads $referenceGenome $i | samtools sort > $output 2> $tmperrorfile
	if [ "$?" -gt 0 ]; then
	    printf "${RED}ERROR - "
	    cat $tmperrorfile
	    printf "${NC}"
...
if [ "$kmerskip" = 0 ]; then
    ...
    do
        ...
        tp=$(count_kmers_kmc $i.tp)
        fn=$(count_kmers_kmc $i.fn)
